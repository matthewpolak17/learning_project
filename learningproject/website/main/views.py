...
from .models import Post, File, Question, Subject, Score
...
def take_subject(request):
    ...
    for score in scores:
        if score.user == request.user:
            if score.subject not in dont_include:
                dont_include.append(score.subject)

    numd = len(dont_include)
    numi = len(subjects)

    if (numd == numi):
        noavail = True
    
    return render(request, 'main/subject/take_subject.html', {"subjects":subjects, "scores":scores, "dont_include":dont_include, "noavail":noavail})
...
@login_required(login_url="/login")
def content(request):
    return render(request, 'main/subject/content.html', {})
